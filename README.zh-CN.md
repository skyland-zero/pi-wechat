...
要求 Pi `>=0.85.0`（扩展依赖 `agent_settled` 生命周期事件）。
...
- 稳定的文本消息桥接
- 出站持久化队列、串行限速与稳定 client ID 重试
- 登录凭证持久化 + 自动备份
- getUpdates 游标与入站队列持久化（重启后续传；崩溃恢复优先保证至少一次处理）
- 会话过期冷却、轮询退避与频控退避
- 出站消息节流
...
- 不要在多个 Pi 进程中同时使用同一份凭证；一个 bot 账号应只由一个进程负责
- 默认只接受登录凭证对应的微信用户；共享多用户还必须显式 opt-in
...
pause.json              持久化会话冷却
inbox.json              入站持久化队列（游标前先落盘）
outbox.json              出站持久化队列（重试复用稳定 client ID；崩溃恢复按至少一次处理）
```
...
桥接消息会进入当前 Pi coding agent，因此微信消息可能触发当前会话启用的工具。
默认只接受登录凭证中的 `ilink_user_id`，其他发送者会被丢弃。
白名单只提供访问控制，不提供每个用户独立的会话/工具隔离。默认拒绝包含多个用户的白名单；
只有明确接受共享上下文时才设置 `PI_WECHAT_ALLOW_SHARED_SESSION=1`。
配置单个授权用户（或显式开启共享会话）可使用：

```bash
PI_WECHAT_ALLOWED_USERS="user-id-1" pi
```

同一 Pi 会话切换到其他微信账号也默认拒绝；只有确认可以共享历史时才设置
`PI_WECHAT_ALLOW_ACCOUNT_SWITCH=1`。

建议为微信桥接使用独立 Pi 会话；桥接处理期间本地终端输入会被拒绝，避免本地回复误发到微信。
API 地址默认仅允许 HTTPS 的微信官方域名；如确实使用受控的企业接入点，可通过
`PI_WECHAT_EXTRA_HOSTS="example.com"` 显式加入精确主机名。
...
这会停止轮询，但会保留待处理消息，下一次 `/wechat-start` 会继续处理。
如需有意清除凭证与队列，请使用 `/wechat-logout`。
...
这会停止桥接，删除当前账号的 `credentials.json` 与 `sync.json`，并清理该账号的待处理入站消息。
其他显式配置账号的持久化消息会保留。
...
| 场景 | 服务端/扩展行为 |
| --- | --- |
| 频率限制（`ret: -2`） | 约 7 条 / 5 分钟。扩展出站节流为最小 5 秒间隔，并以有界退避最多重试 3 次 |
...
长回复会被切成多条消息（每片 4000 字符），**每片都计入频率配额**。如果需要发送大量长回复，
请留意这 7 条 / 5 分钟的限制。默认只在 agent 最终稳定后发送回复；如确需中间块，可设置
`PI_WECHAT_INTERMEDIATE=1`，但自动重试时可能看到重复的中间消息。
...
| 层级 | 行为 | 开关 |
| --- | --- | --- |
| **L1 块级投递** | 默认不发送中间 assistant 块；只在最终 settled 后发送一次，避免重试造成重复 | `PI_WECHAT_INTERMEDIATE=1` |
...
| 兜底 | settled 回复会对照投递账本，只发送尚未投递的残余 | 默认开启 |

扩展等待 `agent_settled`；`agent_end` 之后仍可能发生自动重试、压缩或排队跟进。
